...
export default function Home() {
    ...
    async function send(e) {
      ...
      if(res.status==200 && name && email && password){
        router.push({
          pathname:'/home',
          query:{
            name,
      ...
      <div className="flex">
          <div className="w-6/12 h-screen">
            ...
            <div className="w-3/5 m-auto mt-10">
                <h1 className="flex justify-center pb-10 text-2xl font-medium">Sign Up</h1>
                <label >Name</label><br />
                <input type="text" className="border-2 pr-64 py-1 pl-2 rounded mb-4 shadow-md focus:outline-blue-600" value={name} onChange={e=>setName(e.target.value)}/><br />
                ...
                <button className="bg-blue px-48 py-1 mt-8 bg-blue-500 text-white rounded-md " onClick={send}>SIGN UP</button>
                <h2 className="flex justify-center first-letter mt-8 text-sm text-gray-500">If you already had an account <span> <a className="text-blue-700 pl-2 font-medium cursor-pointer"  onClick={login}> Log In</a></span></h2>
